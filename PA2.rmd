...
Given the graph below, torandic events are clearly most harmful to population 
health.
...
stormOtherHealth <- aggregate(cbind(FATALITIES, INJURIES) ~ EVTYPE, sum, 
        data = stormOtherHealth)
stormOtherHealth <- stormOtherHealth[!(stormOtherHealth$FATALITIES == 0 & 
        stormOtherHealth$INJURIES == 0),]
#Melt data by category to allow for an easy barplot
meltStormOtherHealth <- melt(stormOtherHealth, id="EVTYPE", 
        variable.name="Health_Type", value.name="Number_Of_Incidents")
p2 <- qplot(x=EVTYPE, y=Number_Of_Incidents, fill=Health_Type, 
        data=meltStormOtherHealth, geom="bar", stat="identity") + 
        coord_flip() + xlab("Event Type")

grid.arrange(p1, p2, ncol = 1, main = "Health Events (Top= All Event Types, 
        Bottom=Fine Grain Detail of Other category)")
```

As shown in the table below, see the highest number of incidents by type, it continues 
```{r, results='asis'}
...
meltFatalities <- fatalities[order(fatalities$Number_Of_Incidents, decreasing=TRUE),]

#Start new data framr with just number of incidnets and make categories the rowname
...
meltInjuries <- injuries[order(injuries$Number_Of_Incidents, decreasing=TRUE),]
